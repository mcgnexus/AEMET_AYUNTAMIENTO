...
export function LightningPanel({ data }: { data?: LightningData | null }) {
  ...
  return (
    <div className={`rounded-lg border px-3 py-2 ${isActive ? `${bg} ${border} ${text}` : `${bg} ${border} ${text}`} transition-colors`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          <span className={`text-base ${isActive ? "animate-pulse" : ""}`}>{icon}</span>
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider">
              {isActive ? "Rayos Activos" : "Rayos"}
            </p>
        ...
        {data.source === "blitzortung" && (
          <div className="text-right">
            <p className="text-[10px] font-semibold">
              {data.strikeCount > 0 ? `${data.strikeCount} rayo${data.strikeCount !== 1 ? "s" : ""}` : "Sin rayos"}
            </p>
